...
class ProjectCreation(models.TransientModel):
    ...
    picking_type_id = fields.Many2one(string='Tipo de operación')
    location_id = fields.Many2one(string='Ubicación de origen')
    location_dest_id = fields.Many2one(string='Ubicación de destino')
    scheduled_date = fields.Datetime(string='Fecha programada')
    origin = fields.Many2one(string='Documento origen')
    task_id = fields.Many2one(string='Tarea de origen')
