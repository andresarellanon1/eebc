            ...
            if line.product_pricelist_id.currency_id != line.order_id.target_currency_id:
                ...
                if not product_pricelist:
                    raise ValidationError(
                        f"No se pudo calcular el cambio de divisa. No se encontró una lista de precios para el producto ‘{line.product_template_id.name}’ con la moneda ‘{line.order_id.target_currency_id.name}’. Sin esta equivalencia, no es posible realizar el cambio de divisa. Por favor, elimine la línea de producto que causa este error de validación o cree la lista de precios correspondiente."
                    )
        ...
        for line in self:
            ...
            return self.env["product.pricelist.line"].search(
                [
                    ...
                    ("pricelist_id.location_id", "=", self.env.company.id)
                ],
                limit=1
            )
