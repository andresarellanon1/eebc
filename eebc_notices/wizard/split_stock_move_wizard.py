...
class SplitStockMoveWizard(models.TransientModel):
    ...
    def action_split_stock_move(self):
        self.ensure_one()
        stock_move = self.stock_move_id.id
    ...
    def split_stock_move(self, move_id, split_quantities):
        original_move = self.env['stock.move'].browse(move_id)
        
        if original_move.product_uom_qty <= 0:
            raise ValueError("La cantidad original debe ser mayor que 0.")
        
        if sum(split_quantities) != original_move.product_uom_qty:
            raise ValueError("La suma de las cantidades divididas no coincide con la cantidad original.")
