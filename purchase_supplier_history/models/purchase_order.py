...
class PurchaseOrder(models.Model):
    ...
    def button_confirm(self):
        ...
        for order in self:
            ...
            for order_line in order.order_line:
                self._check_quantity_divisibility(order_line)
        return res
    ...
    def _check_quantity_divisibility(self, order_line):
        product_id = order_line.product_id
        partner_id = order_line.order_id.partner_id
        supplierinfo = None

        if product_id and partner_id:
            if partner_id == product_id.main_supplier_id.partner_id:
                supplierinfo = product_id.main_supplier_id
            else:
                for supplier in product_id.seller_ids:
                    if supplier.partner_id == partner_id:
                        supplierinfo = supplier

            if supplierinfo and supplierinfo.min_qty and order_line.product_qty % supplierinfo.multiplier != 0:
                raise ValidationError(
                    f"Cantidad para producto '{product_id.display_name}' debe ser divisible por el múltiplo ({supplierinfo.multiplier}) del proveedor."
                )
